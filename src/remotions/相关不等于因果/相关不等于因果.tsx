...
export const 相关不等于因果Schema = z.object({});
...
const sceneConfigs = [
    { name: "scene1", duration: calculateScene1Duration() + SCENE_END_PADDING, component: Scene1, label: "喝药痊愈的错觉" },
    { name: "scene2", duration: calculateScene2Duration() + SCENE_END_PADDING, component: Scene2, label: "相关性与因果性的混淆" },
    { name: "scene3", duration: calculateScene3Duration() + SCENE_END_PADDING, component: Scene3, label: "理性看待偏方与传统" },
];

const MAIN_DURATION_相关不等于因果 =
    sceneConfigs.reduce((total, c) => total + c.duration, 0) -
    (sceneConfigs.length - 1) * TRANSITION_DURATION;

export const TOTAL_DURATION_相关不等于因果 =
    COVER_DURATION_FRAMES + MAIN_DURATION_相关不等于因果;
...
export const 相关不等于因果: React.FC<z.infer<typeof 相关不等于因果Schema>> = () => {
    const frame = useCurrentFrame();
