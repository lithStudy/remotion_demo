...
export const 认知偏见幸存者偏差Schema = z.object({});
...
const sceneConfigs = [
    ...
    { name: "scene3", duration: calculateScene3Duration() + SCENE_END_PADDING, component: Scene3, label: "反转：剧毒鸡汤" },
    { name: "scene4", duration: calculateScene4Duration() + SCENE_END_PADDING, component: Scene4, label: "召唤：核心心法" },
];

const MAIN_DURATION_认知偏见_幸存者偏差 =
    sceneConfigs.reduce((total, c) => total + c.duration, 0) -
    (sceneConfigs.length - 1) * TRANSITION_DURATION;

export const TOTAL_DURATION_认知偏见_幸存者偏差 =
    COVER_DURATION_FRAMES + MAIN_DURATION_认知偏见_幸存者偏差;
...
export const 认知偏见幸存者偏差: React.FC<z.infer<typeof 认知偏见幸存者偏差Schema>> = () => {
    const frame = useCurrentFrame();
