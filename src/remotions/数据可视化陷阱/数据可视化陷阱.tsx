...
export const 数据可视化陷阱Schema = z.object({});
...
const sceneConfigs = [
    { name: "scene1", duration: calculateScene1Duration() + SCENE_END_PADDING, component: Scene1, label: "发布会现场的视觉陷阱" },
    { name: "scene2", duration: calculateScene2Duration() + SCENE_END_PADDING, component: Scene2, label: "视觉冲击与洗脑" },
    { name: "scene3", duration: calculateScene3Duration() + SCENE_END_PADDING, component: Scene3, label: "几何骗局的常见手法" },
    { name: "scene4", duration: calculateScene4Duration() + SCENE_END_PADDING, component: Scene4, label: "如何进行图表审计" },
];

const MAIN_DURATION_数据可视化陷阱 =
    sceneConfigs.reduce((total, c) => total + c.duration, 0) -
    (sceneConfigs.length - 1) * TRANSITION_DURATION;

export const TOTAL_DURATION_数据可视化陷阱 =
    COVER_DURATION_FRAMES + MAIN_DURATION_数据可视化陷阱;
...
export const 数据可视化陷阱: React.FC<z.infer<typeof 数据可视化陷阱Schema>> = () => {
    const frame = useCurrentFrame();
    ...
    return (
        <AbsoluteFill>
            ...
            <Sequence from={COVER_DURATION_FRAMES} durationInFrames={MAIN_DURATION_数据可视化陷阱}>
                <AbsoluteFill>
                    <TransitionSeries>
