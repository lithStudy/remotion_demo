...
export const 碎片认知Schema = z.object({});
...
export const sceneConfigs = [
    ...
    { name: "scene3", duration: calculateScene3Duration() + SCENE_END_PADDING, component: Scene3, label: "反转" },
    { name: "scene4", duration: calculateScene4Duration() + SCENE_END_PADDING, component: Scene4, label: "召唤" },
];

export const MAIN_DURATION_碎片认知 =
    sceneConfigs.reduce((total, c) => total + c.duration, 0) -
    (sceneConfigs.length - 1) * TRANSITION_DURATION;

export const TOTAL_DURATION_碎片认知 =
    COVER_DURATION_FRAMES + MAIN_DURATION_碎片认知;
