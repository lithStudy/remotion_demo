...
export const 可证伪性Schema = z.object({});
...
const sceneConfigs = [
    { name: "scene1", duration: calculateScene1Duration() + SCENE_END_PADDING, component: Scene1, label: "引出现象：不可证伪的逻辑陷阱" },
    { name: "scene2", duration: calculateScene2Duration() + SCENE_END_PADDING, component: Scene2, label: "剖析原因：人类对确定性的渴望" },
    { name: "scene3", duration: calculateScene3Duration() + SCENE_END_PADDING, component: Scene3, label: "科学的定义：可证伪性" },
    { name: "scene4", duration: calculateScene4Duration() + SCENE_END_PADDING, component: Scene4, label: "防御锦囊：如何避免被忽悠" },
];

const MAIN_DURATION_可证伪性 =
    sceneConfigs.reduce((total, c) => total + c.duration, 0) -
    (sceneConfigs.length - 1) * TRANSITION_DURATION;

export const TOTAL_DURATION_可证伪性 =
    COVER_DURATION_FRAMES + MAIN_DURATION_可证伪性;
...
export const 可证伪性: React.FC<z.infer<typeof 可证伪性Schema>> = () => {
    const frame = useCurrentFrame();
    ...
    return (
        <AbsoluteFill>
            ...
            <Sequence from={COVER_DURATION_FRAMES} durationInFrames={MAIN_DURATION_可证伪性}>
                <AbsoluteFill>
                    <TransitionSeries>
