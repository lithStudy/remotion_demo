...
def main():
    ...
    # 新格式：item.images 数组，每个元素含 prompt / position / sync_with_segment / image_effect
    # 图片命名规则：{scene_id}_{order}_{position}.png（单图 center 时简写为 {scene_id}_{order}.png）
    items_flat = []
    for scene in scripts_data.get("scenes", []):
        ...
        for item in scene.get("items", []):
            ...
            if images and isinstance(images, list):
                for img in images:
                    p = (img.get("prompt") or "").strip()
                    if not p:
                        continue
                    position = (img.get("position") or "center").strip()
                    # center 且只有一张图时不加后缀，保持向后兼容
                    suffix = None if (position == "center" and len(images) == 1) else position
                    items_flat.append(
                        {
                            "scene_id": scene_id,
                            "order": item["order"],
                ...
                # 兼容旧格式：imagePrompt 字符串或 {left, right} 对象
                ...
                if layout == "SPLIT_COMPARE" and isinstance(raw_prompt, dict):
                    ...
                    if left_p:
                        items_flat.append({"scene_id": scene_id, "order": item["order"], "imagePrompt": left_p, "suffix": "left"})
                    if right_p:
                        items_flat.append({"scene_id": scene_id, "order": item["order"], "imagePrompt": right_p, "suffix": "right"})
                    continue
                prompt = (raw_prompt if isinstance(raw_prompt, str) else "").strip()
                if not prompt:
                    continue
                items_flat.append({"scene_id": scene_id, "order": item["order"], "imagePrompt": prompt, "suffix": None})
