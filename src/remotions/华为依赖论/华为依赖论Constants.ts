...
export const sceneConfigs = [
    { name: "scene1", duration: calculateScene1Duration() + SCENE_END_PADDING, component: Scene1, label: "引入：唯一华为之惧" },
    { name: "scene2", duration: calculateScene2Duration() + SCENE_END_PADDING, component: Scene2, label: "剖析：通信网络并非华为独建" },
    { name: "scene3", duration: calculateScene3Duration() + SCENE_END_PADDING, component: Scene3, label: "剖析：份额分配的双重逻辑" },
    { name: "scene4", duration: calculateScene4Duration() + SCENE_END_PADDING, component: Scene4, label: "命名：压舱石角色" },
    { name: "scene5", duration: calculateScene5Duration() + SCENE_END_PADDING, component: Scene5, label: "反转：份额不等于技术" },
    { name: "scene6", duration: calculateScene6Duration() + SCENE_END_PADDING, component: Scene6, label: "剖析：技术领先的短暂性" },
    { name: "scene7", duration: calculateScene7Duration() + SCENE_END_PADDING, component: Scene7, label: "剖析：安全在于选择" },
    { name: "scene8", duration: calculateScene8Duration() + SCENE_END_PADDING, component: Scene8, label: "反转：华为消失的假设" },
...
export const TOTAL_DURATION_华为依赖论 =
    COVER_DURATION_FRAMES + MAIN_DURATION_华为依赖论;
