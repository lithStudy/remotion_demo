...
export const 认知偏见光环效应Schema = z.object({});
...
export const sceneConfigs = [
    { name: "scene1", duration: calculateScene1Duration() + SCENE_END_PADDING, component: Scene1, label: "引入：光环效应与头衔崇拜" },
    { name: "scene2", duration: calculateScene2Duration() + SCENE_END_PADDING, component: Scene2, label: "剖析：权威崇拜的认知偏差" },
    { name: "scene3", duration: calculateScene3Duration() + SCENE_END_PADDING, component: Scene3, label: "反转：光环效应的潜在危害" },
    { name: "scene4", duration: calculateScene4Duration() + SCENE_END_PADDING, component: Scene4, label: "召唤：对抗光环效应的策略" },
];

export const MAIN_DURATION_认知偏见_光环效应 =
    sceneConfigs.reduce((total, c) => total + c.duration, 0) -
    (sceneConfigs.length - 1) * TRANSITION_DURATION;

export const TOTAL_DURATION_认知偏见_光环效应 =
    COVER_DURATION_FRAMES + MAIN_DURATION_认知偏见_光环效应;
