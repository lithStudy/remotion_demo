...
export const 样本偏差Schema = z.object({});
...
const sceneConfigs = [
    { name: "scene1", duration: calculateScene1Duration() + SCENE_END_PADDING, component: Scene1, label: "深夜刷屏的焦虑" },
    { name: "scene2", duration: calculateScene2Duration() + SCENE_END_PADDING, component: Scene2, label: "算法的视觉霸凌" },
    { name: "scene3", duration: calculateScene3Duration() + SCENE_END_PADDING, component: Scene3, label: "统计学上的偏差" },
    { name: "scene4", duration: calculateScene4Duration() + SCENE_END_PADDING, component: Scene4, label: "防御动作" },
];

const MAIN_DURATION_样本偏差 =
    sceneConfigs.reduce((total, c) => total + c.duration, 0) -
    (sceneConfigs.length - 1) * TRANSITION_DURATION;

export const TOTAL_DURATION_样本偏差 =
    COVER_DURATION_FRAMES + MAIN_DURATION_样本偏差;
...
export const 样本偏差: React.FC<z.infer<typeof 样本偏差Schema>> = () => {
    const frame = useCurrentFrame();
    ...
    return (
        <AbsoluteFill>
            ...
            <Sequence durationInFrames={COVER_DURATION_FRAMES}>
                <StaticCover
                    title="样本偏差"
                    subtitle="破除社交媒体制造的焦虑"
                    themeColor="#2563EB"
                    badge="统计素养 · 理性思考"
                />
            </Sequence>
            <Sequence from={COVER_DURATION_FRAMES} durationInFrames={MAIN_DURATION_样本偏差}>
                <AbsoluteFill>
                    <div
                        style={{
                            position: "absolute",
