...
4. **完整覆盖与原文零修改：**
   - 所有 item 里的 `text` 顺次拼合起来，必须 100% 完整覆盖整个原文本段落，不遗漏任何字。
   - 必须使用原文原句，严禁对原文做任何缩写、改写或重新措辞！

5. **锚点标注法则：**
   - **锚点宜少**：精选锚点词句，大多数 content 不需要锚点。
   - **需要锚点时**：content 条目必须写成对象格式：
     `{{"text": "原文", "anchor": "关键词", "anchorColor": "#颜色", "audioEffect": "音效"}}`
   - **不需要锚点时**：content 条目直接写成普通的单行字符串即可。
6. **锚点颜色含义**：
   - `"#E53E3E"` 危险/负面/警告词
   - `"#FF8C00"` 注意/质疑/反直觉词
   - `"#2B6CB0"` 事实/数据/中性专有名词
   - `"#276749"` 正面/结论/建议词
   - `"#805AD5"` 认知/洞察/抽象概念词
7. **音效**：有 anchor 时配音效：`impact_thud`（冲击类）或 `ping`（提示类）或 `woosh`（转场类）
8. **图片纯视觉**：图片描述只描述视觉画面，绝不含文字内容。
9. **先判叙事再选模板**：为每个 item 先归类叙事类型（单图叙述 / 双图并列 / 对错避坑 / 步骤 / 时间轴 / 多图并列 / 纯文字金句 / 引用 / 对话气泡 / 术语卡 / 揭秘锚点 / 冲击收束），再对照下文模板说明选型；不确定时优先 `CENTER_FOCUS`。
10. **严禁滥用 CENTER_FOCUS**：`CENTER_FOCUS` 仅用于无明显结构化视觉语义时的最后兜底；若出现对比、对错、步骤、时间、数据、进度、术语、引用、对话等信号，必须优先选择对应专用模板。
11. **输出可解释选型**：每个 item 必须包含 `narrativeType`（叙事类型）和 `reasoning`（一句话说明为何选该模板），再给出 `template` 与 `param`。
