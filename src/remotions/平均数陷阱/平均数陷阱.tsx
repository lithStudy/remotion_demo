...
export const 平均数陷阱Schema = z.object({});
...
const sceneConfigs = [
    { name: "scene1", duration: calculateScene1Duration() + SCENE_END_PADDING, component: Scene1, label: "热搜引发的自卑" },
    { name: "scene2", duration: calculateScene2Duration() + SCENE_END_PADDING, component: Scene2, label: "平均数陷阱与中位数" },
    { name: "scene3", duration: calculateScene3Duration() + SCENE_END_PADDING, component: Scene3, label: "关注真实的社会进步" },
];

const MAIN_DURATION_平均数陷阱 =
    sceneConfigs.reduce((total, c) => total + c.duration, 0) -
    (sceneConfigs.length - 1) * TRANSITION_DURATION;

export const TOTAL_DURATION_平均数陷阱 =
    COVER_DURATION_FRAMES + MAIN_DURATION_平均数陷阱;
...
export const 平均数陷阱: React.FC<z.infer<typeof 平均数陷阱Schema>> = () => {
    const frame = useCurrentFrame();
