...
export const 统计素养之相关不等于因果Schema = z.object({});
...
const sceneConfigs = [
    { name: "scene1", duration: calculateScene1Duration() + SCENE_END_PADDING, component: Scene1, label: "幸存者偏差的陷阱" },
    { name: "scene2", duration: calculateScene2Duration() + SCENE_END_PADDING, component: Scene2, label: "大脑的底层BUG" },
    { name: "scene3", duration: calculateScene3Duration() + SCENE_END_PADDING, component: Scene3, label: "对照组的重要性" },
    { name: "scene4", duration: calculateScene4Duration() + SCENE_END_PADDING, component: Scene4, label: "双盲实验的武器" },
    { name: "scene5", duration: calculateScene5Duration() + SCENE_END_PADDING, component: Scene5, label: "防御偏方的武装" },
    { name: "scene6", duration: calculateScene6Duration() + SCENE_END_PADDING, component: Scene6, label: "理性与清醒的价值" },
];

const MAIN_DURATION_统计素养之相关不等于因果 =
    sceneConfigs.reduce((total, c) => total + c.duration, 0) -
    (sceneConfigs.length - 1) * TRANSITION_DURATION;

export const TOTAL_DURATION_统计素养之相关不等于因果 =
    COVER_DURATION_FRAMES + MAIN_DURATION_统计素养之相关不等于因果;
...
export const 统计素养之相关不等于因果: React.FC<z.infer<typeof 统计素养之相关不等于因果Schema>> = () => {
    const frame = useCurrentFrame();
    ...
    return (
        <AbsoluteFill>
            ...
            <Sequence durationInFrames={COVER_DURATION_FRAMES}>
                <StaticCover
                    title="相关≠因果"
                    subtitle="科学认知与避免信息陷阱"
                    themeColor="#2563EB"
                    badge="统计素养 · 理性思考"
                />
            </Sequence>
            <Sequence from={COVER_DURATION_FRAMES} durationInFrames={MAIN_DURATION_统计素养之相关不等于因果}>
                <AbsoluteFill>
                    <div
                        style={{
                            position: "absolute",
