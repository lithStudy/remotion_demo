...
const sceneConfigs = [
    { name: "scene1", duration: calculateScene1Duration() + SCENE_END_PADDING, component: Scene1, label: "引入：读书无用论" },
    { name: "scene2", duration: calculateScene2Duration() + SCENE_END_PADDING, component: Scene2, label: "剖析：幸存者偏差" },
    { name: "scene3", duration: calculateScene3Duration() + SCENE_END_PADDING, component: Scene3, label: "举例：剧毒鸡汤" },
    { name: "scene4", duration: calculateScene4Duration() + SCENE_END_PADDING, component: Scene4, label: "召唤：核心心法" },
    { name: "scene5", duration: calculateScene5Duration() + SCENE_END_PADDING, component: Scene5, label: "反转：认清概率" },
];

const MAIN_DURATION_TEST =
    sceneConfigs.reduce((total, c) => total + c.duration, 0) -
    (sceneConfigs.length - 1) * TRANSITION_DURATION;

export const TOTAL_DURATION_TEST =
    COVER_DURATION_FRAMES + MAIN_DURATION_TEST;
...
export const Test: React.FC<z.infer<typeof TestSchema>> = () => {
    const frame = useCurrentFrame();
    ...
    return (
        <AbsoluteFill>
            ...
            <Sequence durationInFrames={COVER_DURATION_FRAMES}>
                <StaticCover
                    title="test"
                    subtitle="为什么“越努力越焦虑”？别让成功学鸡汤，毁了你的人生"
