...
export const 华为抹黑论Schema = z.object({});
...
export const sceneConfigs = [
    { name: "scene1", duration: calculateScene1Duration() + SCENE_END_PADDING, component: Scene1, label: "引入：抹黑还是事实" },
    { name: "scene2", duration: calculateScene2Duration() + SCENE_END_PADDING, component: Scene2, label: "剖析：抹黑与事实清单" },
    { name: "scene3", duration: calculateScene3Duration() + SCENE_END_PADDING, component: Scene3, label: "反转：罄竹难书，证据在此" },
];

export const MAIN_DURATION_华为抹黑论 =
    sceneConfigs.reduce((total, c) => total + c.duration, 0) -
    (sceneConfigs.length - 1) * TRANSITION_DURATION;

export const TOTAL_DURATION_华为抹黑论 =
    COVER_DURATION_FRAMES + MAIN_DURATION_华为抹黑论;
