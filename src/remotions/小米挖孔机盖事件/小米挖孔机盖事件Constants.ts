...
export const 小米挖孔机盖事件Schema = z.object({});
...
export const sceneConfigs = [
    { name: "scene1", duration: calculateScene1Duration() + SCENE_END_PADDING, component: Scene1, label: "引入：维权公正与信任危机" },
    { name: "scene2", duration: calculateScene2Duration() + SCENE_END_PADDING, component: Scene2, label: "反转：挖孔机盖虚假散热" },
    { name: "scene3", duration: calculateScene3Duration() + SCENE_END_PADDING, component: Scene3, label: "剖析：0.002升力与价值错位" },
    { name: "scene4", duration: calculateScene4Duration() + SCENE_END_PADDING, component: Scene4, label: "反转：老板不懂技术的挡箭牌" },
    { name: "scene5", duration: calculateScene5Duration() + SCENE_END_PADDING, component: Scene5, label: "剖析：信息差历史与物理定律" },
    { name: "scene6", duration: calculateScene6Duration() + SCENE_END_PADDING, component: Scene6, label: "召唤：真诚回归与改进之路" },
];

export const MAIN_DURATION_小米挖孔机盖事件 =
    sceneConfigs.reduce((total, c) => total + c.duration, 0) -
    (sceneConfigs.length - 1) * TRANSITION_DURATION;

export const TOTAL_DURATION_小米挖孔机盖事件 =
    COVER_DURATION_FRAMES + MAIN_DURATION_小米挖孔机盖事件;
